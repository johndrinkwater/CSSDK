...
BOOL CBasePlayer::CanAffordSecondaryAmmo( void )
{
	if( m_pLastSecondaryItem )
	{
		...
		for( int i = 0; i < ARRAYSIZE( g_weaponStruct ); i++ )
		{
			...
			if( pWeaponStruct->id == m_pLastSecondaryItem->m_iId && pWeaponStruct->cost <= m_iAccount )
			{
				return TRUE;
			}
		}
...
void CBasePlayer::DeathSound( void )
{
	...
	{
		case 0 : EMIT_SOUND( ENT( pev ), CHAN_VOICE, "player/die1.wav", VOL_NORM, ATTN_NORM ); break;
		case 1 : EMIT_SOUND( ENT( pev ), CHAN_VOICE, "player/die2.wav", VOL_NORM, ATTN_NORM ); break;
		case 2 : EMIT_SOUND( ENT( pev ), CHAN_VOICE, "player/die3.wav", VOL_NORM, ATTN_NORM ); break;
		case 3 : EMIT_SOUND( ENT( pev ), CHAN_VOICE, "player/death6.wav", VOL_NORM, ATTN_NORM ); break;
	}
}
...
void CBasePlayer::DropPlayerItem ( char *pszItemName )
{
	...
	if( pWeapon )
	{
		...
		if( pWeapon->iItemSlot() == ITEMSLOT_PRIMARY )
		{
		...
		if( !strcmp( STRING( pWeapon->pev->classname ), "weapon_c4" ) )
		{
			...
			for( int i = 1; i <= gpGlobals->maxClients; ++i )
			{
				...
				{
					pPlayer->SetScoreboardAttributes( this );
				}
			}
			...
			if( !g_pGameRules->m_flTeamCount )
			{
				UTIL_LogPrintf( "\"%s<%i><%s><TERRORIST>\" triggered \"Dropped_The_Bomb\"\n",
									STRING( pWeapon->pev->netname ),
									GETPLAYERUSERID( this->edict() ),
									GETPLAYERAUTHID( this->edict() ) );

				g_pGameRules->m_fBombDropped = true;
				...
				{
					if( !FNullEnt( pEntity->edict() ) && pEntity->IsPlayer() && !pEntity->IsDormant() )
					{
						pPlayer = GetClassPtr( (CBasePlayer *)VARS( pEntity->pev ) );

						if( pPlayer->pev->deadflag == DEAD_NO && pPlayer->m_iTeam == TEAM_TERRORIST )
						{
							ClientPrint( pPlayer->pev, HUD_PRINTCENTER, "#Game_bomb_drop", STRING( pPlayer->pev->netname ) );

							MESSAGE_BEGIN( MSG_ONE, gmsgBombDrop, NULL, ENT( pPlayer->pev ) );
		...
		if( iAmmoIndex != -1 && pWeapon->iFlags() & ITEM_FLAG_EXHAUSTIBLE )
		{
...
BOOL CBasePlayer::IsHittingShield( Vector const &vecDir, TraceResult* ptr )
{
	...
	if( IsProtectedByShield() || ptr->iHitgroup == HITGROUP_SHIELD )
		return true;

// TODO: Check this. Return false by default in CS.
// -
// 	Vector2D	vec2LOS;
// 	float		flDot;
// 
...
void CBasePlayer::ResetMaxSpeed()
{
	...
	{
		flSpeed = m_pActiveItem->GetMaxSpeed();
	}
	else
	{
		flSpeed = 240.0;
