...
//CSSDK
void CBaseMonster::BloodSplat( Vector &vecOrigin, Vector &vecDirection, int hitGroup, int speed )
{
	if( hitGroup == HITGROUP_HEAD )
	{
		...
		WRITE_COORD( vecDirection.x );
		WRITE_COORD( vecDirection.y );
		WRITE_COORD( vecDirection.z );
		WRITE_BYTE( 223 );
		WRITE_BYTE( speed + RANDOM_LONG( 0, 100 ) );
		MESSAGE_END();
	}
}
...
void CBaseMonster::RadiusDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int iClassIgnore, int bitsDamageType )
{
	if( flDamage <= 80.0 )
	{
		//::RadiusDamage2( pev->origin, pevInflictor, pevAttacker, flDamage, ( flDamage + 3.0 ) * flDamage, iClassIgnore, bitsDamageType );
	}
	else
	{
...
// CSSDK
Vector CBaseEntity::FireBullets3( Vector vecSrc, Vector vecDirShooting, float flSpread, float flDistance, int iPenetration, int iBulletType, int iDamage, float flRangeModifier, entvars_t *pevAttacker, int paramInt, int shared_rand )
{
